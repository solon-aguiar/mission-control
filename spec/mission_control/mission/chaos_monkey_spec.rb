  ...
  describe 'chaos_for_mission' do
    it 'generates abort_at at "auto_abort_rate"' do
      ...
      for i in (0..auto_abort_rate * 2) do
        abort_at, _ = monkey.chaos_for_mission
    ...
    it 'generates explode_at at "auto_explode_rate"' do
      ...
      for i in (0..auto_explode_rate * 2) do
        _, explode_at = monkey.chaos_for_mission
    ...
    it 'allows either explode_at or abort_at in the same mission' do
      ...
      expect(random).to receive(:rand).exactly(6).times.with(1..auto_abort_rate).and_return(
        auto_abort_rate,
        auto_abort_rate,
        auto_abort_rate,
        auto_abort_rate,
        auto_abort_rate,
        auto_abort_rate + 1 # different number to break the loop
      )
